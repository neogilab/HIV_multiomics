---
title: "Boxplots microbiome derived metabolites"
...
colnames_list <- data.frame(t(colnames_list))
colnames(list_mic_met) <- colnames_list$X1
```
...
```{r}
dge_sign <- read.delim("results/LIMMA_results_metabolites_with_corrected.csv", sep = ",")
```

```{r}
met <- unique(dge_mic_met$BIOCHEMICAL)
```
...
```{r}
table_DGE <- data.frame(Metabolite = met, 
                        LFC_G1G2 = NA, LFC_G1G3 = NA, LFC_G2G3 =NA,
                        LFC_C1_Ctrl = NA, LFC_C2_Ctrl = NA, LFC_C3_Ctrl = NA,
                        FDR_G1G2 = NA, FDR_G1G3 = NA, FDR_G2G3 =NA,
                        FDR_C1_Ctrl = NA, FDR_C2_Ctrl = NA, FDR_C3_Ctrl = NA)
```


```{r}
for (i in 1:nrow(table_DGE)){
  table_DGE[i, 2] <- DGE_2$logFC[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group2-group1"]
  table_DGE[i, 3] <- DGE_2$logFC[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group3-group1"]
  table_DGE[i, 4] <- DGE_2$logFC[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group3-group2"]
  ...
  table_DGE[i, 8] <- DGE_2$adj.P.Val[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group2-group1"]
  table_DGE[i, 9] <- DGE_2$adj.P.Val[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group3-group1"]
  table_DGE[i, 10] <- DGE_2$adj.P.Val[DGE_2$BIOCHEMICAL == DGE$BIOCHEMICAL[i] & DGE_2$Comp == "group3-group2"]
...
write.csv(table_DGE_filt, "processing/processing_microbione_derived_metabolomics_corrected_model.csv")
name_met <- rowSums(is.na(table_DGE_filt[,-1])) != ncol(table_DGE_filt[,-1])
table_DGE_filt <- table_DGE_filt[name_met,]

write.csv(table_DGE_filt, "processing/processing_microbione_derived_metabolomics_corrected_model_filt.csv")
```
...
```{r}
pdf("results/figures/figure_2_heatmap_3_layers_integration_microbiome_derived_metabolites_corrected.pdf", width = 16, height = 7)
ht = Heatmap(t(data_3), column_order = sort(colnames(t(data_3))),  col = col_fun1, show_column_names = FALSE, column_split = cluster, row_split = data_2$under_group, column_title = c("Ctrl",1, 2, 3), 
             ...
             right_annotation = rowAnnotation(Ctrl_HIV = data_2$LFC_HC_HIV, 
                                              C2vsC1 = data_2$LFC_G1G2,
                                              ...
                                              col = list(Ctrl_HIV = col_fun_2, 
                                                         C2vsC1 = col_fun_2, 
                                                         C3vsC1 = col_fun_2, 
                                                         C3vsC2 = col_fun_2,
                                                         C1vsCtrl = col_fun_2, 
                                                         C2vsCtrl = col_fun_2, 
                                                         C3vsCtrl = col_fun_2), show_legend = TRUE))
draw(ht)
dev.off()
```
```{r}
x <- data_2[,-c(1, 126:131)]
rownames(x) <- data_2$Metabolite
x$label <- "MDM_all_comp"
write.csv(x, "processing/list_microbiome_derived_metabolites_heatmap_all_comparisons_corrected.csv")
```
...
write.xlsx(correlations, "processing/table_edges_first_neigbors_corrected.xlsx")

write.xlsx(m[m$name %in% c(correlations$feat1, correlations$feat2),], "processing/table_nodes_first_neigbors_corrected.xlsx")
```
