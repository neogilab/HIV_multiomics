...
## clinical data
```{r}
clinical_data <- read.csv("data/patients_microbiome_project.csv")
```
...
## microbiome
```{r}
data_mi <- read.csv("processing/relative_abundance_table.csv")
...
```
```{r}
write.csv(data_mi, "processing/data_mi_filt_SNF.csv")
write.csv(data_met, "processing/data_met_filt_SNF.csv")
write.csv(data_lip, "processing/data_lip_filt_SNF.csv")
```
...
write.csv(table_cluters, "processing/cluster_SNF_3_omics_3_clusters.csv")
```
...
```{r}
pdf("results/figures/NMI_matrix_selection_clusters_new_figure.pdf")
col = colorRamp2(c(0.2, 0.4,1), c("white", "pink","red"))
