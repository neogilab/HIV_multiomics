...
```{r}
extra <- read.xlsx("/home/flomik/Desktop/Code-PHD/3_layers_integration/data/clinical_data_aging_study_2.xlsx", 1)
extra <- select(extra, COCOMO_ID, drug_1, drug_3, drug_bis)
extra$drug_1 <- as.vector(extra$drug_1)
extra$drug_1[is.na(extra$drug_1)] <- "unknown"
extra$drug_3 <- as.vector(extra$drug_3)
extra$drug_3[is.na(extra$drug_3)] <- "unknown"

extra$drug_1[extra$drug_1 %in% c("TAF", "TDF")] <- "TAF_TDF"
extra$drug_bis <- as.vector(extra$drug_bis)
...
```{r}
table_numeric$parameter[table_numeric$pval_kruskal < 0.05 & !is.na(table_numeric$pval_kruskal)]
```
```{r}
data_numeric$curr_smoking
```

```{r}
data_numeric$ART1_prev_dur.x
```
...
```
```{r}
table_factor$Parameter[table_factor$pval < 0.05 & !is.na(table_factor$pval)]
```
