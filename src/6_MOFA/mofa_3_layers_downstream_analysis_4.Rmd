...
```{r}
conv_met <- read.csv("processing/biochemical_super_path.csv")
names(conv_met)[1] <- "met"
conv_lip <- read.csv("processing/lipids_superpathways.csv")
names(conv_lip)[1] <- "item"
tax_table <- read.csv("processing/tax_table_COCOMO.csv")
...
```{r}
MOFAobject <- load_model("results/mofa_model_4")
```
...
pdf("results/figures/MOFA_overview_model_4.pdf")
plot_data_overview(MOFAobject)
dev.off()
```
...
pdf("results/figures/correlation_factors_model_4.pdf")
plot_factor_cor(MOFAobject)
dev.off()
```
...
pdf("results/figures/MOFA_variance_model_4.pdf")
plot_variance_explained(MOFAobject)
dev.off()
```
## Total variance
...
pdf("results/figures/MOFA_variance_modeL_4_2.pdf")
plot_variance_explained(MOFAobject, plot_total = T)[[2]]
dev.off()
```
...
x <- data.frame(correlate_factors_with_covariates(MOFAobject, 
  covariates = c("cluster", "hypertension", "Transm_mode", "VAT", "SAT","CD4_CD8", "METS", "AGE", "BMI", "Combined_therapy", "GENDER", "X3rd_Drug"), 
  return_data = TRUE))

x$Factor <- rownames(x)

x2 <- melt(x, id.vars = c("Factor"))
x2 <- x2[x2$value != 0,]

pdf("results/figures/MOFA_covariate_association_model_4.pdf")
correlate_factors_with_covariates(MOFAobject, 
...
x2$value <- NULL
write.csv(x2, "processing/MOFA_correlation_clinical_parameters.csv", row.names = FALSE, quote = FALSE)
```
...
```{r}
pdf("results/figures/weight_distribution_factors.pdf")
...
```{r}
pdf("results/figures/heatmap_F1_model_4.pdf")
...
```{r}
pdf("results/figures/heatmap_all_factors_model_4_spearman_distance_2.pdf", width = 15, height = 15)
...
```{r}
pdf("results/figures/heatmap_all_factors_model_4_spearman_distance.pdf")
data <- cbind(data_met, data_lip, data_mi)
...
library(ggvenn)
pdf("results/figures/overlap_F1_F2_lipids_model_4.pdf")
ggvenn(
  list_lip, 
...
library(ggvenn)
pdf("results/figures/overlap_F1_F2_F5_met_model_4.pdf")
ggvenn(
  list_met, 
  fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
  stroke_size = 0.5, set_name_size = 4
